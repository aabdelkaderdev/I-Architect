...
- Wheels are available for the following platforms:
  ...
  - Wheels are tested on all Python versions currently marked as “Supported”
    on <https://devguide.python.org/versions/>, currently 3.9, 3.10, 3.11, 3.12, 3.13 and 3.14.
...
- There are no **mandatory** external dependencies. However, some optional feature are available only if additional components are installed:
  ...
  - [Pillow](https://pypi.org/project/Pillow/) is required for [`Pixmap.pil_save()`](pixmap.html#Pixmap.pil_save "Pixmap.pil_save") and [`Pixmap.pil_tobytes()`](pixmap.html#Pixmap.pil_tobytes "Pixmap.pil_tobytes").
  - [fontTools](https://pypi.org/project/fonttools/) is required for [`Document.subset_fonts()`](document.html#Document.subset_fonts "Document.subset_fonts").
...
[Tesseract Language Packs](ocr/tesseract-language-packs.html#tesseract-language-packs) for other languages must be installed separately, and the `tessdata` folder must be specified to PyMuPDF as described above, for OCR to work with those languages.
