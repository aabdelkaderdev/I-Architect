...
problems](first_steps.html#security). Please be sure to carefully follow the different proposed
...
server doesnât send the same cookies as the ones sent by the users. Extensions
...
`pdf_variant` Python parameter of [`HTML.write_pdf`](api_reference.html#weasyprint.HTML.write_pdf "weasyprint.HTML.write_pdf").
...
forbidden in A-2. The âuâ part of the variant indicates that the PDF text is
...
CSS property to `auto` on them. In this case, as for browsers, youâll have to
manually override the default style set by the user agent stylesheet. Reading
[the stylesheet set by the âpdf-forms option](https://github.com/Kozea/WeasyPrint/blob/main/weasyprint/css/html5_ua_form.css)
...
feature doesnât work for you, please check that this feature is actually
...
If you donât want to attach your files using HTML tags, you can also use the
...
[`weasyprint.Attachment`](api_reference.html#weasyprint.Attachment "weasyprint.Attachment") class.
...
Itâs also possible to cache images on disk instead of keeping them in memory.
...
but itâs not the main culprit here. [Optimization is not the main goal of
WeasyPrint](going_further.html#why-python) and it may lead to unbearable long rendering times.

First of all: WeasyPrintâs performance gets generally better with time. You can
...
Most errors (unsupported CSS property, missing imageâ¦) are not fatal and will
not prevent a document from being rendered. WeasyPrint uses the [`logging`](https://docs.python.org/3/library/logging.html#module-logging "(in Python v3.14)")
...
See the documentation of the [`logging`](https://docs.python.org/3/library/logging.html#module-logging "(in Python v3.14)") module for details.