...
Invoices, tickets, leaflets, diplomas, documentation, booksâ¦ All these
...
basic features. Thatâs why projects such as [wkhtmltopdf](https://wkhtmltopdf.org/) and [PagedJS](https://www.pagedjs.org/) have been
...
in WeasyPrintâs documentation one month after the beginning:

> Are we crazy? Yes. But not that much. Each modern web browser did take many
> developersâ many years of work to get where they are now, but WeasyPrintâs
> scope is much smaller: there is no user-interaction, no JavaScript, no live
> rendering (the document doesnât changed after it was first parsed) and no
> quirks mode (we donât need to support every broken page of the web.)
>
...
> much quicker than âLetâs build a rendering engine!â may seem.
...
Speed is not WeasyPrintâs main goal. Web rendering is a very complex task, and
...
This chapter is a high-level overview of WeasyPrintâs source code. For more
details, see the various docstrings or even the code itself. When in doubt,
feel free to [ask](support.html#support)!
...
6. For each page, the boxes are re-ordered to observe stacking rules, and are
   drawn on a PDF page.
7. Metadata âsuch as document information, attachments, embedded files,
   hyperlinks, and PDF trim and bleed boxesâ are added to the PDF.

### Parsing HTML

Not much to see here. The [`HTML`](api_reference.html#weasyprint.HTML "weasyprint.HTML") class handles step 1 and
...
As with HTML, CSS stylesheets are parsed in the [`CSS`](api_reference.html#weasyprint.CSS "weasyprint.CSS") class
...
After that and still in the `css` package, the [cascade](https://www.w3.org/TR/CSS21/cascade.html)
(thatâs the C in CSS!) applies the stylesheets to the element tree.
...
the one with the highest *weight* wins. Weights are based on the stylesheetâs
[origin](api_reference.html#stylesheet-origins), `!important` markers, selector
specificity and source order. Missing values are filled in through
*inheritance* (from the parent element) or the propertyâs *initial value*,
...
pseudo-element is a string for eg. `::first-line` selectors, or [`None`](https://docs.python.org/3/library/constants.html#None "(in Python v3.14)")
for ânormalâ elements. Style dict objects are dicts mapping property names to
the computed values. (The return value is not the dict itself, but a
convenience `style_for` function for accessing it.)

### Formatting Structure

The [visual formatting model](https://www.w3.org/TR/CSS21/visuren.html) explains how *elements* (from the ElementTree
...
represent all these boxes. We wonât go into the details here, see the module
...
During the layout the documentâs content is, well, laid out on pages.
...
According to the [box model](https://www.w3.org/TR/CSS21/box.html), each box has rectangular margin, border,
padding and content areas:

While `box.style` contains computed values, the [used values](https://www.w3.org/TR/CSS21/cascade.html#used-value) are set as
...
[stacking rules](https://www.w3.org/TR/CSS21/zindex.html) such as the `z-index` property. The result is a tree of
*stacking contexts*.
