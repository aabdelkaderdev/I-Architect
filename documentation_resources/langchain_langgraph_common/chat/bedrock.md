...
**Anthropic models on Bedrock**For Anthropic models specifically, you can use `ChatAnthropicBedrock` which extends `ChatAnthropic` and provides the same API while running on AWS Bedrock. See the [ChatAnthropicBedrock section](#chatanthropicbedrock) below for details.
...
### [​](#streaming-tool-calls-and-structured-output) Streaming tool calls and structured output
