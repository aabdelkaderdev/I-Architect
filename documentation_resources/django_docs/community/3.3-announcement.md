<!-- Source: https://www.django-rest-framework.org/community/3.3-announcement -->
...
- A [forms API](../../topics/html-and-forms/), allowing serializers to be rendered as HTML forms.
- Django 1.9 support.
- A [`JSONField` serializer field](../../api-guide/fields/#jsonfield), corresponding to Django 1.9's Postgres `JSONField` model field.
- Browsable API support [via AJAX](https://github.com/encode/ajax-form), rather than server side request overloading.
...
This brings our supported versions into line with Django's [currently supported versions](https://www.djangoproject.com/download/#supported-versions)
...
- To support form based `PUT` and `DELETE`, or to support form content types such as JSON, you should now use the [AJAX forms](https://github.com/encode/ajax-form) javascript library. This replaces the previous 'method and content type overloading' that required significant internal complexity to the request class.
- The `accept` query parameter is no longer supported by the default content negotiation class. If you require it then you'll need to [use a custom content negotiation class](../../topics/browser-enhancements/#url-based-accept-headers).
- The custom `HTTP_X_HTTP_METHOD_OVERRIDE` header is no longer supported by default. If you require it then you'll need to [use custom middleware](../../topics/browser-enhancements/#http-header-based-method-overriding).
