<!-- Source: https://www.django-rest-framework.org/community/3.1-announcement -->
...
A guide to the headline features follows. For full details, see [the pagination documentation](../../api-guide/pagination/).
...
We've made it [easier to build versioned APIs](../../api-guide/versioning/). Built-in schemes for versioning include both URL based and Accept header based variations.
...
REST framework now includes a built-in set of translations, and [supports internationalized error responses](../../topics/internationalization/). This allows you to either change the default language, or to allow clients to specify the language via the `Accept-Language` header.
...
Note that the structure of the error responses is still the same. We still have a `detail` key in the response. If needed you can modify this behavior too, by using a [custom exception handler](../../api-guide/exceptions/#custom-exception-handling).
...
For more details, see the [internationalization documentation](../../topics/internationalization/).
