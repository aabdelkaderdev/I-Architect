...
For more details see the [HTML & Forms](../../topics/html-and-forms/) documentation.
...
Format may either be the special string `'iso-8601'`, which indicates that [ISO 8601](https://www.w3.org/TR/NOTE-datetime) style intervals should be used (eg `'P4DT1H15M20S'`), or `'django'` which indicates that Django interval format `'[DD] [HH:[MM:]]ss[.uuuuuu]'` should be used (eg: `'4 1:15:20'`).
...
Django's regular [FILE\_UPLOAD\_HANDLERS](https://docs.djangoproject.com/en/stable/ref/settings/#std:setting-FILE_UPLOAD_HANDLERS) are used for handling uploaded files.
...
For further examples on `HiddenField` see the [validators](../validators/) documentation.
