...
2. Search for or follow the link to [Nunito](https://fonts.google.com/specimen/Nunito), and select "**Get font**."
3. Search for or follow the link to [Space Mono](https://fonts.google.com/specimen/Space+Mono), and select "**Get font**."
4. To get a link to a style sheet for your font files, in the upper-right corner, select the shopping bag (*shopping\_bag*), and then select "*code* **Get embed code**."
5. On the right, in the first code block, copy the `href` URL from the third link, and paste it into a new tab.
   ...
   ```
6. Go to your new tab and visit the URL.
...
7. To store the portion of the style sheet you'll need in later steps, copy the font-face declarations that are prefixed with the `/* latin */` comment, and paste them into a text file.
...
1. In `your_repository/`, create a `.streamlit/config.toml` file:
   ...
   └── .streamlit/
       └── config.toml
   ```
2. To define your alternative fonts, in `.streamlit/config.toml`, paste each `@font-face` declaration, and change each one into a `[[theme.fontFaces]]` table.
...
3. To set your alternative fonts as the default font for your app, in `.streamlit/config.toml`, add the following text:
...
[*arrow\_back*Previous: Use external font files](/develop/tutorials/configuration-and-theming/external-fonts)[*arrow\_forward*Next: Use static font files](/develop/tutorials/configuration-and-theming/static-fonts)
