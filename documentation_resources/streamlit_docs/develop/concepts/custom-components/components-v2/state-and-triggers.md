<!-- Source: https://docs.streamlit.io/develop/concepts/custom-components/components-v2/state-and-triggers -->
...
| Callback execution | Only if `setStateValue()` *changed* the value | Every time `setTriggerValue()` is called |
...
[*arrow\_forward*View the full radial menu example](/develop/concepts/custom-components/components-v2/examples/radial-menu)
...
On the Python side, the `on_confirmed_change` callback runs each time the trigger fires. Because trigger values are transient, `result.confirmed` is only `True` during the rerun caused by the trigger—it reverts to `None` on subsequent reruns:
...
[*arrow\_forward*View the full danger button example](/develop/concepts/custom-components/components-v2/examples/danger-button)
...
[*arrow\_back*Previous: Mounting](/develop/concepts/custom-components/components-v2/mount)[*arrow\_forward*Next: Bidirectional communication](/develop/concepts/custom-components/components-v2/communicate)
