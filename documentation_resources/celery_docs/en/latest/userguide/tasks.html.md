<!-- Source: https://docs.celeryq.dev/en/latest/userguide/tasks.html -->
...
until that message has been [acknowledged](../glossary.html#term-acknowledged) by a worker. A worker can reserve
...
Ideally task functions should be [idempotent](../glossary.html#term-idempotent): meaning
...
If your task is idempotent you can set the [`acks_late`](#Task.acks_late "Task.acks_late") option
to have the worker acknowledge the message *after* the task returns
instead. See also the FAQ entry [Should I use retry or acks\_late?](../faq.html#faq-acks-late-vs-retry).
...
even when [`acks_late`](#Task.acks_late "Task.acks_late") is enabled. This behavior is intentional
as…
...
consider enabling the [`task_reject_on_worker_lost`](configuration.html#std-setting-task_reject_on_worker_lost) setting.
...
like adding a timeout to a web request using the <https://pypi.org/project/requests/> library:
...
[Time limits](workers.html#worker-time-limits) are convenient for making sure all
...
was advised to enable the [`-Ofair`](../reference/cli.html#cmdoption-celery-worker-O) command-line
argument to the **celery worker**. However, as of version 4.0,
-Ofair is now the default scheduling strategy. See [Prefetch Limits](optimizing.html#optimizing-prefetch-limit)
for more information, and for the best performance route long-running and
short-running tasks to dedicated workers ([Automatic routing](routing.html#routing-automatic)).
...
the [`app.task()`](../reference/celery.html#celery.Celery.task "celery.Celery.task") decorator:
...
There are also many [options](#task-options) that can be set for the task,
...
> The task decorator is available on your [`Celery`](../reference/celery.html#celery.Celery "celery.Celery") application instance,
> if you don’t know what this is then please read [First Steps with Celery](../getting-started/first-steps-with-celery.html#first-steps).
>
> If you’re using Django (see [First steps with Django](../django/first-steps-with-django.html#django-first-steps)), or you’re the author
...
Bound tasks are needed for retries (using [`app.Task.retry()`](../reference/celery.app.task.html#celery.app.task.Task.retry "celery.app.task.Task.retry")),
...
[Management Command-line Utilities (inspect/control)](monitoring.html#monitoring-control) section of the User Guide.
...
[`app.gen_task_name()`](../reference/celery.html#celery.Celery.gen_task_name "celery.Celery.gen_task_name"). Continuing with the example, celery.py
may contain:
...
Make sure that your [`app.gen_task_name()`](../reference/celery.html#celery.Celery.gen_task_name "celery.Celery.gen_task_name") is a pure function: meaning
that for the same input it must always return the same output.

## 

[`app.Task.request`](../reference/celery.app.task.html#celery.app.task.Task.request "celery.app.task.Task.request") contains information and state
...
group:
:   The unique id of the task’s [group](canvas.html#canvas-group), if this task is a member.
...
:   The original ETA of the task (if any).
    This is in UTC time (depending on the [`enable_utc`](configuration.html#std-setting-enable_utc)
    setting).
...
:   Additional message delivery information. This is a mapping
    ...
    task. Used by for example [`app.Task.retry()`](../reference/celery.app.task.html#celery.app.task.Task.retry "celery.app.task.Task.retry")
...
utc:
:   Set to true the caller has UTC enabled ([`enable_utc`](configuration.html#std-setting-enable_utc)).
...
see [`worker_redirect_stdouts`](configuration.html#std-setting-worker_redirect_stdouts)).
...
use the [`setup_logging`](signals.html#std-signal-setup_logging) signal:
...
When using [`task_protocol`](configuration.html#std-setting-task_protocol) 2 or higher (default since 4.0), you can
...
[`app.Task.retry()`](../reference/celery.app.task.html#celery.app.task.Task.retry "celery.app.task.Task.retry") can be used to re-execute the task,
...
instance (see [States](#task-states)).
...
The [`app.Task.retry()`](../reference/celery.app.task.html#celery.app.task.Task.retry "celery.app.task.Task.retry") call will raise an exception so any
code after the retry won’t be reached. This is the [`Retry`](../reference/celery.exceptions.html#celery.exceptions.Retry "celery.exceptions.Retry")
...
- An `exc` argument wasn’t given.
  ...
  > In this case the [`MaxRetriesExceededError`](../reference/celery.exceptions.html#celery.exceptions.MaxRetriesExceededError "celery.exceptions.MaxRetriesExceededError")
...
[`default_retry_delay`](../reference/celery.app.task.html#celery.app.task.Task.default_retry_delay "celery.app.task.Task.default_retry_delay")
attribute. By default this is set to 3 minutes. Note that the
unit for setting the delay is in seconds (int or float).

You can also provide the countdown argument to [`retry()`](../reference/celery.app.task.html#celery.app.task.Task.retry "celery.app.task.Task.retry") to
...
autoretry\_for argument in the [`app.task()`](../reference/celery.html#celery.Celery.task "celery.Celery.task") decorator:
...
If you want to specify custom arguments for an internal [`retry()`](../reference/celery.app.task.html#celery.app.task.Task.retry "celery.app.task.Task.retry")
call, pass retry\_kwargs argument to [`app.task()`](../reference/celery.html#celery.Celery.task "celery.Celery.task") decorator:
...
makes it easy. Just specify the [`retry_backoff`](#Task.retry_backoff "Task.retry_backoff") argument, like this:
...
:   A boolean, or a number. If this option is set to `True`, autoretries
    ...
    [`retry_jitter`](#Task.retry_jitter "Task.retry_jitter"), if it is enabled.)
...
:   A boolean. [Jitter](https://en.wikipedia.org/wiki/Jitter) is used to introduce randomness into
    ...
    value calculated by [`retry_backoff`](#Task.retry_backoff "Task.retry_backoff") is treated as a maximum,
...
:   A list/tuple of exception classes. These exceptions won’t be autoretried.
    :   This allows to exclude some exceptions that match [`autoretry_for`](#Task.autoretry_for "Task.autoretry_for") but for which you don’t want a retry.
...
:   The name the task is registered as.
    ...
    See also [Names](#task-names).

Task.request
:   If the task is being executed this will contain information
    about the current request. Thread local storage is used.

    See [Task Request](#task-request-info).

Task.max\_retries
:   Only applies if the task calls `self.retry` or if the task is decorated
    with the [autoretry\_for](#task-autoretry) argument.

    The maximum number of attempted retries before giving up.
    If the number of retries exceeds this value a [`MaxRetriesExceededError`](../reference/celery.exceptions.html#celery.exceptions.MaxRetriesExceededError "celery.exceptions.MaxRetriesExceededError")
    exception will be raised.

    Note

    You have to call [`retry()`](../reference/celery.app.task.html#celery.app.task.Task.retry "celery.app.task.Task.retry")
...
:   Set the rate limit for this task type (limits the number of tasks
    ...
    Default is the [`task_default_rate_limit`](configuration.html#std-setting-task_default_rate_limit) setting:
...
:   Don’t store task state. Note that this means you can’t use
    [`AsyncResult`](../reference/celery.result.html#celery.result.AsyncResult "celery.result.AsyncResult") to check if the task is ready,
...
:   A string identifying the default serialization
    method to use. Defaults to the [`task_serializer`](configuration.html#std-setting-task_serializer)
    setting. Can be pickle, json, yaml, or any custom
    serialization methods that have been registered with
    `kombu.serialization.registry`.

    Please see [Serializers](calling.html#calling-serializers) for more information.

Task.compression
:   A string identifying the default compression scheme to use.

    Defaults to the [`task_compression`](configuration.html#std-setting-task_compression) setting.
    ...
    Please see [Compression](calling.html#calling-compression) for more information.
...
:   The result store backend to use for this task. An instance of one of the
    ...
    defined by the [`result_backend`](configuration.html#std-setting-result_backend) setting.
...
:   If set to `True` messages for this task will be acknowledged
    **after** the task has been executed, not *just before* (the default
    ...
    [idempotent](../glossary.html#term-idempotent).

    The global default can be overridden by the [`task_acks_late`](configuration.html#std-setting-task_acks_late)
    setting.
...
:   If `True` the task will report its status as “started”
    ...
    [`task_track_started`](configuration.html#std-setting-task_track_started) setting.

See also

The API reference for [`Task`](../reference/celery.app.task.html#celery.app.task.Task "celery.app.task.Task").
...
different strengths and weaknesses (see [Result Backends](#task-result-backends)).
...
[`STARTED`](#std-state-STARTED) state at some point).

There are also sets of states, like the set of
`FAILURE_STATES`, and the set of [`READY_STATES`](../reference/celery.states.html#std-state-READY_STATES).

The client uses the membership of these sets to decide whether
the exception should be re-raised ([`PROPAGATE_STATES`](../reference/celery.states.html#std-state-PROPAGATE_STATES)), or whether
the state can be cached (it can if the task is ready).

You can also define [Custom states](#custom-states).
...
[Task result backend settings](configuration.html#conf-result-backend)
...
persistent messages using the [`result_persistent`](configuration.html#std-setting-result_persistent) setting.
...
Not reported by default, to enable please see [`app.Task.track_started`](../reference/celery.app.task.html#celery.app.task.Task.track_started "celery.app.task.Task.track_started").
...
Use [`update_state()`](../reference/celery.app.task.html#celery.app.task.Task.update_state "celery.app.task.Task.update_state") to update a task’s state:.
...
The task may raise [`Ignore`](../reference/celery.exceptions.html#celery.exceptions.Ignore "celery.exceptions.Ignore") to force the worker to ignore the
...
The task may raise [`Reject`](../reference/celery.exceptions.html#celery.exceptions.Reject "celery.exceptions.Reject") to reject the task message using
AMQPs `basic_reject` method. This won’t have any effect unless
[`Task.acks_late`](#Task.acks_late "Task.acks_late") is enabled.
...
The [`Retry`](../reference/celery.exceptions.html#celery.exceptions.Retry "celery.exceptions.Retry") exception is raised by the `Task.retry` method
to tell the worker that the task is being retried.

## 

All tasks inherit from the [`app.Task`](../reference/celery.app.task.html#celery.app.task.Task "celery.app.task.Task") class.
The [`run()`](../reference/celery.app.task.html#celery.app.task.Task.run "celery.app.task.Task.run") method becomes the task body.
...
- `after_return` executes when the task reaches a terminal state.
  ...
  a hook that fires on every attempt, use the [`task_postrun`](signals.html#std-signal-task_postrun) signal.

#### Available handlers

before\_start(*self*, *task\_id*, *args*, *kwargs*)
:   Run by the worker before the task starts executing.

    Note

    This handler **blocks** the task: the `run()` method will *not* begin
...
:   Handler called after the task returns.
    ...
    [`task_postrun`](signals.html#std-signal-task_postrun) signal.
...
Upon receiving a message to run a task, the [worker](workers.html#guide-workers)
creates a [`request`](../reference/celery.worker.request.html#celery.worker.request.Request "celery.worker.request.Request") to represent such
demand.

Custom task classes may override which request class to use by changing the
attribute [`celery.app.task.Task.Request`](../reference/celery.app.task.html#celery.app.task.Task.Request "celery.app.task.Task.Request"). You may either assign the
...
strongly recommend to inherit from [`celery.worker.request.Request`](../reference/celery.worker.request.html#celery.worker.request.Request "celery.worker.request.Request").

When using the [pre-forking worker](workers.html#worker-concurrency), the methods
[`on_timeout()`](../reference/celery.worker.request.html#celery.worker.request.Request.on_timeout "celery.worker.request.Request.on_timeout") and
[`on_failure()`](../reference/celery.worker.request.html#celery.worker.request.Request.on_failure "celery.worker.request.Request.on_failure") are executed in the main
worker process. An application may leverage such facility to detect failures
which are not detected using [`celery.app.task.Task.on_failure()`](../reference/celery.app.task.html#celery.app.task.Task.on_failure "celery.app.task.Task.on_failure").
...
[`imports`](configuration.html#std-setting-imports) setting.

The [`app.task()`](../reference/celery.html#celery.Celery.task "celery.Celery.task") decorator is responsible for registering your task
...
[`ignore_result`](../reference/celery.app.task.html#celery.app.task.Task.ignore_result "celery.app.task.Task.ignore_result") option, as storing results
...
Results can even be disabled globally using the [`task_ignore_result`](configuration.html#std-setting-task_ignore_result)
setting.
...
1. Global [`task_ignore_result`](configuration.html#std-setting-task_ignore_result)
2. [`ignore_result`](../reference/celery.app.task.html#celery.app.task.Task.ignore_result "celery.app.task.Task.ignore_result") option
3. Task execution option `ignore_result`

### More optimization tips

You find additional optimization tips in the
[Optimizing Guide](optimizing.html#guide-optimizing).
...
different [`signature()`](../reference/celery.html#celery.signature "celery.signature")’s.
You can read about chains and other powerful constructs
at [Canvas: Designing Work-flows](../getting-started/next-steps.html#designing-workflows).
...
[`delay_on_commit()`](../reference/celery.contrib.django.task.html#celery.contrib.django.task.DjangoTask.delay_on_commit "celery.contrib.django.task.DjangoTask.delay_on_commit") instead:
