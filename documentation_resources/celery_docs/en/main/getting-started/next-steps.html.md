<!-- Source: https://docs.celeryq.dev/en/main/getting-started/next-steps.html -->

This document describes the current stable version of Celery (5.6).
For development docs,
[go here](https://docs.celeryq.dev/en/main/getting-started/next-steps.html).

# Next Steps

The [First Steps with Celery](first-steps-with-celery.html#first-steps) guide is intentionally minimal. In this guide
...
[User Guide](../userguide/index.html#guide)
...
In this module you created our [`Celery`](../reference/celery.html#celery.Celery "celery.Celery") instance (sometimes
...
- The `broker` argument specifies the URL of the broker to use.
  ...
  > See [Choosing a Broker](first-steps-with-celery.html#celerytut-broker) for more information.
- The `backend` argument specifies the result backend to use.
  ...
  >
  > See [Keeping Results](first-steps-with-celery.html#celerytut-keeping-results) for more information.
...
The **celery** program can be used to start the worker (you need to run the worker in the directory above
...
the [`-b`](../reference/cli.html#cmdoption-celery-b) option.
...
the [`celery worker -c`](../reference/cli.html#cmdoption-celery-worker-c) option.
...
Eventlet, Gevent, and running in a single thread (see [Concurrency](../userguide/concurrency/index.html#concurrency)).
...
the [Monitoring and Management guide](../userguide/monitoring.html#guide-monitoring).
...
and prioritization, all described in the [Routing Guide](../userguide/routing.html#guide-routing).
...
These options are described in more detailed in the [Workers Guide](../userguide/workers.html#guide-workers).

#### Stopping the worker

To stop the worker simply hit `Control`-`c`. A list of signals supported
by the worker is detailed in the [Workers Guide](../userguide/workers.html#guide-workers).

#### In the background

In production you’ll want to run the worker in the background,
described in detail in the [daemonization tutorial](../userguide/daemonizing.html#daemonizing).

The daemonization scripts uses the **celery multi** command to
...
For more examples see the [`multi`](../reference/celery.bin.multi.html#module-celery.bin.multi "celery.bin.multi") module in the API
reference.

#### About the [`--app`](../reference/cli.html#cmdoption-celery-A) argument

The [`--app`](../reference/cli.html#cmdoption-celery-A) argument specifies the Celery app instance
...
With [`--app=proj`](../reference/cli.html#cmdoption-celery-A):
...
These three methods - `delay()`, `apply_async()`, and applying
...
[Calling User Guide](../userguide/calling.html#guide-calling).

Every task invocation will be given a unique identifier (an UUID) – this
is the task id.

The `delay` and `apply_async` methods return an [`AsyncResult`](../reference/celery.result.html#celery.result.AsyncResult "celery.result.AsyncResult")
instance, which can be used to keep track of the tasks execution state.
But for this you need to enable a [result backend](../userguide/tasks.html#task-result-backends) so that
...
for that Celery uses dedicated event messages (see [Monitoring and Management Guide](../userguide/monitoring.html#guide-monitoring)).
...
[`task_track_started`](../userguide/configuration.html#std-setting-task_track_started) setting is enabled, or if the
...
To read more about task states you should see the [States](../userguide/tasks.html#task-states) section
in the tasks user guide.

Calling tasks is described in detail in the
[Calling Guide](../userguide/calling.html#guide-calling).
...
above already does that (see the backend argument to [`Celery`](../reference/celery.html#celery.Celery "celery.Celery")).

Let’s look at some examples:

#### Groups

A [`group`](../reference/celery.html#celery.group "celery.group") calls a list of tasks in parallel,
...
Be sure to read more about work-flows in the [Canvas](../userguide/canvas.html#guide-canvas) user
guide.
...
The [`task_routes`](../userguide/configuration.html#std-setting-task_routes) setting enables you to route tasks by name
...
specifying the [`celery worker -Q`](../reference/cli.html#cmdoption-celery-worker-Q) option:
...
power of AMQP routing, see the [Routing Guide](../userguide/routing.html#guide-routing).
...
using the [`--destination`](../reference/cli.html#cmdoption-celery-inspect-d) option.
...
The **celery inspect** command contains commands that
...
Then there’s the **celery control** command, which contains
...
You can read more about the **celery** command and monitoring
in the [Monitoring Guide](../userguide/monitoring.html#guide-monitoring).
...
configure that using the [`timezone`](../userguide/configuration.html#std-setting-timezone) setting:
...
for throughput then you should read the [Optimizing Guide](../userguide/optimizing.html#guide-optimizing).

## 

Now that you have read this document you should continue
to the [User Guide](../userguide/index.html#guide).

There’s also an [API reference](../reference/index.html#apiref) if you’re so inclined.