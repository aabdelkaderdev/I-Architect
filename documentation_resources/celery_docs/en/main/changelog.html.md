<!-- Source: https://docs.celeryq.dev/en/main/changelog.html -->

This document describes the current stable version of Celery (5.6).
For development docs,
[go here](https://docs.celeryq.dev/en/main/changelog.html).

# Change history

This document contains change notes for bugfix & new features
in the main branch & 5.6.x series, please see [What’s new in Celery 5.6 (Recovery)](history/whatsnew-5.6.html#whatsnew-5-6) for
...
- Fix recursive WorkController instantiation in DjangoWorkerFixup + AttributeError when pool\_cls is a string (#10045)
...
- docs #5410 – Document confirm\_publish broker transport option (#10016)
...
- Docs: clarify result\_extended vs periodic task metadata and show headers[“periodic\_task\_name”] example (#10030)
- Stop importing pytest\_subtests (#10032)
...
See [What’s new in Celery 5.6 (Recovery)](history/whatsnew-5.6.html#whatsnew-5-6) for a complete overview or read the main highlights below.
...
New configuration option [`worker_eta_task_limit`](userguide/configuration.html#std-setting-worker_eta_task_limit) to prevent out-of-memory
...
- [`task_create_missing_queue_type`](userguide/configuration.html#std-setting-task_create_missing_queue_type): Sets the queue type for auto-created
  queues (e.g., `quorum`, `classic`)
- [`task_create_missing_queue_exchange_type`](userguide/configuration.html#std-setting-task_create_missing_queue_exchange_type): Sets the exchange type for
  auto-created queues
...
- Fix regex pattern in version parsing and remove duplicate entry in \_\_all\_\_ (#9978)
...
- Feature: Add support credential\_provider to Redis Backend (#9879)
- Celery.timezone: try tzlocal.get\_localzone() before using LocalTimezone (#9862)
- Run integration tests on Python 3.14 (#9903)
- Fix arithmetic overflow for MSSQL result backend (#9904)
- Add documentation for task\_id param for apply\_async function (#9906)
...
- Makes \_on\_retry return a float as required to be used as errback on retry\_over\_time (#9741)
...
- fix: (#9773) task\_id must not be empty with chain as body of a chord (#9774)
- Update setup.py to fix deprecation warning (#9771)
- Adds integration test for chord\_unlock bug when routed to quorum/topic queue (#9766)
- Add xfail test for default queue/exchange fallback ignoring task\_default\_\* settings (#9765)
...
- Fix #9738 : Add root\_id and parent\_id to .apply() (#9784)
- Replace DelayedDelivery connection creation to use context manager (#9793)
- Fix #9794: Pydantic integration fails with \_\_future\_\_.annotations. (#9795)
...
- Remove importlib\_metadata leftovers (#9791)
...
- Migrated from [useblacksmith/build-push-action@v1](mailto:useblacksmith/build-push-action%40v1) to [useblacksmith/setup-docker-builder@v1](mailto:useblacksmith/setup-docker-builder%40v1) in the CI (#9846)
- Remove incorrect example (#9854)
- Revert “Use Django DB max age connection setting” (#9824)
- Fix pending\_result memory leak (#9806)
...
- Add worker\_eta\_task\_limit configuration to manage ETA task memory usage (#9853)
...
- Use EX\_OK instead of literal zero (#9684)
- Make wheel metadata reproducible (#9687)
- let celery install from kombu dependencies for better align (#9696)
- Fix stamping documentation to clarify stamped\_headers key is optional in visitor methods (#9697)
- Support apply\_async without queue argument on quorum queues (#9686)
- Updated rabbitmq doc about using quorum queues with task routes (#9707)
- Add: Dumper Unit Test (#9711)
- Add unit test for event.group\_from (#9709)
- refactor: add beat\_cron\_starting\_deadline documentation warning (#9712)
...
- get\_exchange-unit-test (#9710)
- ISSUE-9704: Update documentation of result\_expires, filesystem backend is supported (#9716)
...
- Remove setup\_logger from COMPAT\_MODULES (#9668)
...
See [What’s new in Celery 5.5 (Immunity)](history/whatsnew-5.5.html#whatsnew-5-5) for a complete overview or read the main highlights below.
...
[#2134](https://github.com/celery/kombu/pull/2134) and integrated in Celery via
[#9526](https://github.com/celery/celery/pull/9526).
...
[New Year’s Security Incident](https://github.com/celery/celery/discussions/9525).
...
- chore(utils/time.py): rename \_is\_ambigious -> \_is\_ambiguous (#9248)
...
- Added missing “ [|oc-sponsor-3|](#id247)” in README (#9326)
...
- Skip test\_pool\_restart\_import\_modules on PyPy due to test issue (#9352)
...
- fix(perform\_pending\_operations): prevent task duplication on shutdown… (#9348)
...
- Pass timeout and confirm\_timeout to producer.publish() (#9374)
...
- Allow custom hostname for celery\_worker in celery.contrib.pytest / celery.contrib.testing.worker (#9405)
...
- Document usage of broker\_native\_delayed\_delivery\_queue\_type (#9419)
...
- Prevent event\_mask from being overwritten. (#9432)
- Update pytest to 8.3.4 (#9444)
- Prepare for (pre) release: v5.5.0rc3 (#9450)
- Bugfix: SIGQUIT not initiating cold shutdown when task\_acks\_late=False (#9461)
...
- worker\_max\_memory\_per\_child: kilobyte is 1024 bytes (#9553)
- Fix formatting in quorum queue docs (#9555)
- Bump cryptography from 44.0.0 to 44.0.1 (#9556)
- Fix the send\_task method when detecting if the native delayed delivery approach is available (#9552)
...
- #9597: Ensure surpassing Hard Timeout limit when task\_acks\_on\_failure\_or\_timeout is False rejects the task (#9626)
...
See [What’s new in Celery 5.5 (Immunity)](history/whatsnew-5.5.html#whatsnew-5-5) or read the main highlights below.
...
- New [documentation](getting-started/backends-and-brokers/rabbitmq.html#using-quorum-queues).
- New [`broker_native_delayed_delivery_queue_type`](userguide/configuration.html#std-setting-broker_native_delayed_delivery_queue_type) configuration option.
...
The soft shutdown is disabled by default, and can be enabled by setting the new configuration option [`worker_soft_shutdown_timeout`](userguide/configuration.html#std-setting-worker_soft_shutdown_timeout).
...
unless the new configuration option [`worker_enable_soft_shutdown_on_idle`](userguide/configuration.html#std-setting-worker_enable_soft_shutdown_on_idle) is set to True. This is useful for workers
that are idle, waiting on ETA tasks to be executed that still want to enable the soft shutdown anyways.

The soft shutdown can replace the cold shutdown when using a broker with a visibility timeout mechanism, like [Redis](getting-started/backends-and-brokers/redis.html#broker-redis)
or [SQS](getting-started/backends-and-brokers/sqs.html#broker-sqs), to enable a more graceful cold shutdown procedure, allowing the worker enough time to re-queue tasks that were not
...
- New [documentation](userguide/workers.html#worker-stopping) for each shutdown type.
- New [`worker_soft_shutdown_timeout`](userguide/configuration.html#std-setting-worker_soft_shutdown_timeout) configuration option.
- New [`worker_enable_soft_shutdown_on_idle`](userguide/configuration.html#std-setting-worker_enable_soft_shutdown_on_idle) configuration option.

#### REMAP\_SIGTERM

The `REMAP_SIGTERM` “hidden feature” has been tested, [documented](userguide/workers.html#worker-remap-sigterm) and is now officially supported.
...
For more info, see the new pydantic example and PR [#9023](https://github.com/celery/celery/pull/9023) by @mathiasertl.

After upgrading to this version, please share your feedback on the new Pydantic support.

#### Redis Broker Stability Improvements

The root cause of the Redis broker instability issue has been [identified and resolved](https://github.com/celery/kombu/pull/2007)
...
- [`task_default_queue_type`](userguide/configuration.html#std-setting-task_default_queue_type)
- [`worker_detect_quorum_queues`](userguide/configuration.html#std-setting-worker_detect_quorum_queues)
...
- Revert “Added Blacksmith.sh to the Sponsors section in the README” (#9324)
- Added Blacksmith.sh to the Sponsors section in the README (#9325)
- Added missing “ [|oc-sponsor-3|](#id249)” in README (#9326)
- Use Blacksmith SVG logo (#9327)
- Updated Blacksmith SVG logo (#9328)
- Revert “Updated Blacksmith SVG logo” (#9329)
- Update pymongo to 4.10.0 (#9330)
...
- chore(ci): Enhance CI with workflow\_dispatch for targeted debugging and testing (#8826)
...
- Bump pre-commit/action from 3.0.0 to 3.0.1 (#8835)
- Support moto 5.0 (#8838)
- Another fix for link\_error signatures being dict`s instead of `Signature s (#8841)
- Bump codecov/codecov-action from 3 to 4 (#8831)
...
- Correct type of request in task\_revoked documentation (#8616)
- update docs docker image (#8618)
- Fixed RecursionError caused by giving config\_from\_object nested mod… (#8619)
...
- Document need for [CELERY\_](#id251) prefix on CLI env vars (#8469)
