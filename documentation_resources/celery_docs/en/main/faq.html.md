<!-- Source: https://docs.celeryq.dev/en/main/faq.html -->

This document describes the current stable version of Celery (5.6).
For development docs,
[go here](https://docs.celeryq.dev/en/main/faq.html).
...
see [Bundles](getting-started/introduction.html#bundles).
...
space, see the [Optimizing](userguide/optimizing.html#guide-optimizing) guide for more information.
...
[Serializers](userguide/security.html#security-serializers).
...
See [Backends and Brokers](getting-started/backends-and-brokers/index.html#brokers) for more information.
...
[`worker`](reference/celery.bin.worker.html#module-celery.bin.worker "celery.bin.worker") is an implementation of Celery in Python. If the
...
Transaction Model and Locking](https://dev.mysql.com/doc/refman/5.1/en/innodb-transaction-model.html) in the MySQL user manual.

(Thanks to Honza Kral and Anton Tsigularov for this solution)

### 

**Answer:** See [MySQL is throwing deadlock errors, what can I do?](#mysql-is-throwing-deadlock-errors-what-can-i-do),
or [Why is Task.delay/apply\*/the worker just hanging?](#why-is-task-delay-apply-the-worker-just-hanging).

### 

**Answer:** If you’re using the database backend for results, and in particular
using MySQL, see [MySQL is throwing deadlock errors, what can I do?](#mysql-is-throwing-deadlock-errors-what-can-i-do).
...
**Answer:** See [MySQL is throwing deadlock errors, what can I do?](#mysql-is-throwing-deadlock-errors-what-can-i-do).
Thanks to [@@howsthedotcom](https://github.com/@howsthedotcom/).
...
**Answer:** See [Why won’t my Task run?](#why-won-t-my-task-run).
...
you have to use the AMQP API or the **celery amqp** utility:
...
[`--purge`](reference/cli.html#cmdoption-celery-worker-purge) option enabled to purge messages
...
This will give you a [`AsyncResult`](reference/celery.result.html#celery.result.AsyncResult "celery.result.AsyncResult") instance
...
[`app.AsyncResult`](reference/celery.result.html#celery.result.AsyncResult "celery.result.AsyncResult"):
...
For the task messages you can set the [`task_serializer`](userguide/configuration.html#std-setting-task_serializer)
setting to “json” or “yaml” instead of pickle.

Similarly for task results you can set [`result_serializer`](userguide/configuration.html#std-setting-result_serializer).

For more details of the formats used and the lookup order when
checking what format to use for a task see [Serializers](userguide/calling.html#calling-serializers)

### 

**Answer**: Some AMQP brokers supports using SSL (including RabbitMQ).
You can enable this using the [`broker_use_ssl`](userguide/configuration.html#std-setting-broker_use_ssl) setting.

It’s also possible to add additional encryption and security to messages,
if you have a need for this then you should contact the mailing-list.
...
the Celery services (**celery worker**, **celery beat**,
**celeryev**, etc) as an unprivileged user is recommended.
...
Running [`worker`](reference/celery.bin.worker.html#module-celery.bin.worker "celery.bin.worker") with the [`-E`](reference/cli.html#cmdoption-celery-worker-E)
...
to lower this value by configuring the [`result_expires`](userguide/configuration.html#std-setting-result_expires)
setting.

If you don’t use the results for a task, make sure you set the
ignore\_result option:
...
**Answer**: No. It used to be supported by <https://pypi.org/project/Carrot/> (our old messaging library)
but isn’t currently supported in <https://pypi.org/project/Kombu/> (our new messaging library).
...
**Answer**: See the [`broker_pool_limit`](userguide/configuration.html#std-setting-broker_pool_limit) setting.
The connection pool is enabled by default since version 2.5.

### [`subprocess`](https://docs.python.org/dev/library/subprocess.html#module-subprocess "(in Python v3.15)") returns `None`

There’s a **sudo** configuration option that makes it illegal
for process without a tty to run **sudo**:
...
**Answer**: Yes, use [`app.send_task()`](reference/celery.html#celery.Celery.send_task "celery.Celery.send_task").
...
use the [`Celery.signature()`](reference/celery.html#celery.Celery.signature "celery.Celery.signature") method:
...
For more information see [Task Request](userguide/tasks.html#task-request-info).

If you don’t have a reference to the task instance you can use
[`app.current_task`](reference/celery.html#celery.Celery.current_task "celery.Celery.current_task"):
...
[`current_worker_task`](reference/celery.html#celery.Celery.current_worker_task "celery.Celery.current_worker_task"):
...
Both [`current_task`](reference/celery.html#celery.Celery.current_task "celery.Celery.current_task"), and [`current_worker_task`](reference/celery.html#celery.Celery.current_worker_task "celery.Celery.current_worker_task") can be
`None`.

### 

**Answer**: Yes, use the task\_id argument to `Task.apply_async()`:
...
**Answer**: Yes, but please see note in the sidebar at [Basics](userguide/tasks.html#task-basics).
...
See [Canvas: Designing Work-flows](userguide/canvas.html) for more information.

### 

**Answer**: Yes, Use [`result.revoke()`](reference/celery.result.html#celery.result.AsyncResult.revoke "celery.result.AsyncResult.revoke"):
...
using the [`-n`](reference/cli.html#cmdoption-celery-worker-n) argument to
[`worker`](reference/celery.bin.worker.html#module-celery.bin.worker "celery.bin.worker"):
...
See [Routing Tasks](userguide/routing.html) for more information.
...
You can use the [`--disable-prefetch`](reference/cli.html#cmdoption-celery-worker-disable-prefetch)
flag (or set [`worker_disable_prefetch`](userguide/configuration.html#std-setting-worker_disable_prefetch) to `True`) so that a worker
...
[Prefetch Limits](userguide/optimizing.html#optimizing-prefetch-limit).

### 

**Answer**: Yes, you can use the Django database scheduler, or you can
create a new schedule subclass and override
[`is_due()`](reference/celery.schedules.html#celery.schedules.schedule.is_due "celery.schedules.schedule.is_due"):
...
is catch-able with the [`try`](https://docs.python.org/dev/reference/compound_stmts.html#try "(in Python v3.15)") block. The AMQP transaction isn’t used
for these errors: **if the task raises an exception it’s still acknowledged!**

The acks\_late setting would be used when you need the task to be
...
are doing can still enable acks\_late (and in the future hopefully
...
combine that with acks\_late if that level of reliability
is required.
...
[periodic tasks](userguide/periodic-tasks.html#guide-beat) should be preferred.

See [ETA and Countdown](userguide/calling.html#calling-eta) for more details.
...
You should never stop [`worker`](reference/celery.bin.worker.html#module-celery.bin.worker "celery.bin.worker") with the `KILL` signal
...
<https://pypi.org/project/setproctitle/> module:
...
[Stopping the worker](userguide/workers.html#worker-stopping)

### 

**Answer**: Yes, please see [Daemonization](userguide/daemonizing.html#daemonizing).
