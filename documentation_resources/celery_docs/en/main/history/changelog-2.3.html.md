<!-- Source: https://docs.celeryq.dev/en/main/history/changelog-2.3.html -->

This document describes the current stable version of Celery (5.6).
For development docs,
[go here](https://docs.celeryq.dev/en/main/history/changelog-2.3.html).
...
- Monkey patching `sys.stdout` could result in the worker
  crashing if the replacing object didn’t define `isatty()`
  (Issue #477).
- `CELERYD` option in `/etc/default/celeryd` shouldn’t
...
- Improved Contributing guide.
  ...
  > [Contributing Gudie](../contributing.html#contributing).
  >
...
- purge/discard\_all wasn’t working correctly (Issue #455).
...
- The [`task-sent`](../userguide/monitoring.html#std-event-task-sent) event was missing from the event reference.
...
- The `CELERY_AMQP_TASK_RESULT_EXPIRES` setting didn’t work,
...
- Results are now disabled by default.
  ...
  > ([`celery.backends.base.DisabledBackend`](../internals/reference/celery.backends.base.html#celery.backends.base.DisabledBackend "celery.backends.base.DisabledBackend")). Saving state is simply an
  > no-op, and AsyncResult.wait(), .result, .state, etc. will raise
  > a [`NotImplementedError`](https://docs.python.org/dev/library/exceptions.html#NotImplementedError "(in Python v3.15)") telling the user to configure the result backend.
  >
  > For help choosing a backend please see [Result Backends](../userguide/tasks.html#task-result-backends).
  >
  ...
  > Note
  >
  > For <https://pypi.org/project/django-celery/> users the default backend is
...
- Automatic connection pool support.
  ...
  >
  > A limit of `None` or 0 means no limit, and connections will be
...
- Introducing Chords (taskset callbacks).
  ...
  > ```
  >
  > Please read the [Chords section in the user guide](../userguide/canvas.html#canvas-chord), if you
  > want to know more.
- Time limits can now be set for individual tasks.
  ...
  > at runtime using the `time_limit()` remote control command:
  >
  > ```
...
- Redis backend configuration directive names changed to include the
  :   `CELERY_` prefix.
      ...
      > | --- | --- |
      > | REDIS\_HOST | CELERY\_REDIS\_HOST |
      > | REDIS\_PORT | CELERY\_REDIS\_PORT |
      > | REDIS\_DB | CELERY\_REDIS\_DB |
      > | REDIS\_PASSWORD | CELERY\_REDIS\_PASSWORD |
      >
...
- Can now use SSL when sending error e-mails by enabling the
  [`EMAIL_USE_SSL`](http://docs.djangoproject.com/en/dev/ref/settings/#std-setting-EMAIL_USE_SSL "(in Django v6.1)") setting.
...
- TaskPool.apply\_async: Keyword arguments `callbacks` and `errbacks`
