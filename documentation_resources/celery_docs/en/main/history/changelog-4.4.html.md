<!-- Source: https://docs.celeryq.dev/en/main/history/changelog-4.4.html -->

This document describes the current stable version of Celery (5.6).
For development docs,
[go here](https://docs.celeryq.dev/en/main/history/changelog-4.4.html).

# Change history

This document contains change notes for bugfix & new features
in the 4.4.x series, please see [What’s new in Celery 4.4 (Cliffs)](whatsnew-4.4.html#whatsnew-4-4) for
...
- Make AsynPool’s proc\_alive\_timeout configurable (#5476)
...
- Fix task serializer being ignored with task\_always\_eager=True (#5549)
- Fix task.replace to work in .apply() as well as `.apply\_async() (#5540)
- Fix sending of worker\_process\_init signal for solo worker (#5562)
...
- Added `acks_on_failure_or_timeout` as a setting instead of
  a task only option.

  This was missing from the original PR but now added for completeness.

  Contributed by **Omer Katz**
- Added the [`task_received`](../userguide/signals.html#std-signal-task_received) signal.

  Contributed by **Omer Katz**
- Fixed a crash of our CLI that occurred for everyone using Python < 3.6.

  The crash was introduced in [acd6025](https://github.com/celery/celery/commit/acd6025b7dc4db112a31020686fc8b15e1722c67)
  by using the [`ModuleNotFoundError`](https://docs.python.org/dev/library/exceptions.html#ModuleNotFoundError "(in Python v3.15)") exception which was introduced
  in Python 3.6.

  Contributed by **Omer Katz**
- Fixed a crash that occurred when using the Redis result backend
  while the [`result_expires`](../userguide/configuration.html#std-setting-result_expires) is set to None.

  Contributed by **Toni Ruža** & **Omer Katz**
- Added support the [DNS seedlist connection format](https://docs.mongodb.com/manual/reference/connection-string/#dns-seedlist-connection-format)
...
- Replace the msgpack-python package with msgpack.
  ...
  See our [important notes](whatsnew-4.3.html#v430-important) for this release
...
- **Canvas**: `celery.chain.apply()` does not ignore keyword arguments anymore when
  applying the chain.

  Contributed by **Korijn van Golen**
- **Result Set**: Don’t attempt to cache results in a [`celery.result.ResultSet`](../reference/celery.result.html#celery.result.ResultSet "celery.result.ResultSet").

  During a join, the results cache was populated using [`celery.result.ResultSet.get()`](../reference/celery.result.html#celery.result.ResultSet.get "celery.result.ResultSet.get"), if one of the results
  contains an exception, joining unexpectedly failed.

  The results cache is now removed.

  Contributed by **Derek Harland**
- **Application**: [`celery.Celery.autodiscover_tasks()`](../reference/celery.html#celery.Celery.autodiscover_tasks "celery.Celery.autodiscover_tasks") now attempts to import the package itself
  when the related\_name keyword argument is None.

  Contributed by **Alex Ioannidis**
- **Windows Support**: On Windows 10, stale PID files prevented celery beat to run.
  We now remove them when a [`SystemExit`](https://docs.python.org/dev/library/exceptions.html#SystemExit "(in Python v3.15)") is raised.

  Contributed by **:github\_user:`na387`**
- **Task**: Added the new [`task_acks_on_failure_or_timeout`](../userguide/configuration.html#std-setting-task_acks_on_failure_or_timeout) setting.

  Acknowledging SQS messages on failure or timing out makes it impossible to use
  dead letter queues.

  We introduce the new option acks\_on\_failure\_or\_timeout,
...
- **Canvas**: Support chords with [`task_always_eager`](../userguide/configuration.html#std-setting-task_always_eager) set to True.

  Contributed by **Axel Haustant**
- **Result Backend**: Optionally store task properties in result backend.

  Setting the [`result_extended`](../userguide/configuration.html#std-setting-result_extended) configuration option to True enables
...
- **New Result Backend**: Added the Azure Block Blob Storage result backend.
  ...
  setting the AZUREBLOCKBLOB\_URL environment variable to
...
- **Task**: `celery.app.task.update_state()` now accepts keyword arguments.
...
- Gracefully handle consumer `kombu.exceptions.DecodeError`.
...
- **Django**: Remove old djcelery loader.
  ...
  Contributed by **Asif Saif Uddin**
- **Result Backend**: [`celery.worker.request.Request`](../reference/celery.worker.request.html#celery.worker.request.Request "celery.worker.request.Request") now passes
  [`celery.app.task.Context`](../reference/celery.app.task.html#celery.app.task.Context "celery.app.task.Context") to the backend’s store\_result functions.
...
- **Asynpool**: Fixed deadlock when closing socket.
  ...
  Upon attempting to close a socket, `celery.concurrency.asynpool.AsynPool`
...
- **Celery Beat**: Correctly consider timezone when calculating timestamp.
  ...
  Contributed by **:github\_user:`yywing`**
- **Celery Beat**: [`celery.beat.Scheduler.schedules_equal()`](../reference/celery.beat.html#celery.beat.Scheduler.schedules_equal "celery.beat.Scheduler.schedules_equal") can now handle
  either arguments being a None value.

  Contributed by **:github\_user:` ratson`**
- **Documentation/Sphinx**: Fixed Sphinx support for shared\_task decorated functions.
...
- **Application**: Added configuration options to allow separate multiple apps
  ...
  The newly added [`event_exchange`](../userguide/configuration.html#std-setting-event_exchange) and [`control_exchange`](../userguide/configuration.html#std-setting-control_exchange)
...
- **Result Backend**: Forget parent result metadata when forgetting
  ...
  Contributed by **:github\_user:`tothegump`**
- **Task** Store task arguments inside [`celery.exceptions.MaxRetriesExceededError`](../reference/celery.exceptions.html#celery.exceptions.MaxRetriesExceededError "celery.exceptions.MaxRetriesExceededError").

  Contributed by **Anthony Ruhier**
- **Result Backend**: Added the [`result_accept_content`](../userguide/configuration.html#std-setting-result_accept_content) setting.
...
- **Command Line**: **celery report** now reports kernel version along
...
- **Canvas**: Complex canvases with error callbacks no longer raises an [`AttributeError`](https://docs.python.org/dev/library/exceptions.html#AttributeError "(in Python v3.15)").

  Very complex canvases such as [this](https://github.com/merchise/xopgi.base/blob/6634819ad5c701c04bc9baa5c527449070843b71/xopgi/xopgi_cdr/cdr_agent.py#L181)
  no longer raise an [`AttributeError`](https://docs.python.org/dev/library/exceptions.html#AttributeError "(in Python v3.15)") which prevents constructing them.
...
- **Command Line**: Added proper error messages in cases where app cannot be loaded.
  ...
  Contributed by **Omer Katz**
- **Task**: Added the [`task_default_priority`](../userguide/configuration.html#std-setting-task_default_priority) setting.
...
- **Result Backend**: Fix memory leak.
  ...
  after adding full weakref support for Python 2 in [vine](https://github.com/celery/vine/tree/v1.2.0).
  More details can be found in [celery/celery#4839](https://github.com/celery/celery/pull/4839).
...
- **Task Execution**: Fixed roundtrip serialization for eager tasks.
  ...
  is present in the call to [`celery.app.task.Task.apply_async()`](../reference/celery.app.task.html#celery.app.task.Task.apply_async "celery.app.task.Task.apply_async").
  ...
  We now continue to use (in order) the serializer argument to [`celery.app.task.Task.apply_async()`](../reference/celery.app.task.html#celery.app.task.Task.apply_async "celery.app.task.Task.apply_async"),
  if present, or the Producer’s serializer if not None.
  If the Producer’s serializer is None,
  it will use the Celery app’s task\_serializer configuration entry as the serializer.

  Contributed by **Brett Jackson**
- **Redis Result Backend**: The [`celery.backends.redis.ResultConsumer`](../internals/reference/celery.backends.redis.html#celery.backends.redis.RedisBackend.ResultConsumer "celery.backends.redis.ResultConsumer") class no longer assumes
...
- **Task**: Added the [`task_inherit_parent_priority`](../userguide/configuration.html#std-setting-task_inherit_parent_priority) setting.
  ...
  Contributed by **:github\_user:`madprogrammer`**
- **Canvas**: Added the [`result_chord_join_timeout`](../userguide/configuration.html#std-setting-result_chord_join_timeout) setting.
