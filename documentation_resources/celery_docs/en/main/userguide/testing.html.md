<!-- Source: https://docs.celeryq.dev/en/main/userguide/testing.html -->

This document describes the current stable version of Celery (5.6).
For development docs,
[go here](https://docs.celeryq.dev/en/main/userguide/testing.html).
...
> - Smoke / Production: Using [pytest-celery](https://pypi.org/project/pytest-celery/) >= 1.0.0
...
The eager mode enabled by the [`task_always_eager`](configuration.html#std-setting-task_always_eager) setting
...
If you want to enable this functionality, have a look at [`task_store_eager_result`](configuration.html#std-setting-task_store_eager_result).
...
Celery also makes a <https://pypi.org/project/pytest/> plugin available that adds fixtures that you can
...
dictionary returned by the `celery_worker_parameters()` fixture.
...
To enable heartbeats modify the `celery_worker_parameters()` fixture:
...
to configure the `celery_app()`, and `celery_session_app()` fixtures.
...
Celery app. In contrast to `celery_config()`, these are directly passed to
when instantiating [`Celery`](../reference/celery.html#celery.Celery "celery.Celery").
...
[`WorkController`](../reference/celery.worker.html#celery.worker.WorkController "celery.worker.WorkController") when it is instantiated.

The config returned by your fixture will then be used
to configure the `celery_worker()`, and `celery_session_worker()`
fixtures.
...
if something tries to access the default or current\_app, an exception
is raised.
