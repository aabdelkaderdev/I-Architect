<!-- Source: https://docs.celeryq.dev/en/main/userguide/application.html -->

This document describes the current stable version of Celery (5.6).
For development docs,
[go here](https://docs.celeryq.dev/en/main/userguide/application.html).
...
The configuration is available as [`app.conf`](../reference/celery.app.utils.html#celery.app.utils.Settings "celery.app.utils.Settings"):
...
> 3. The default configuration ([`celery.app.defaults`](../reference/celery.app.defaults.html#module-celery.app.defaults "celery.app.defaults")).

You can even add new default sources by using the [`app.add_defaults()`](../reference/celery.html#celery.Celery.add_defaults "celery.Celery.add_defaults")
method.

See also

Go to the [Configuration reference](configuration.html#configuration) for a complete
listing of all the available settings, and their default values.

### `config_from_object`

The [`app.config_from_object()`](../reference/celery.html#celery.Celery.config_from_object "celery.Celery.config_from_object") method loads configuration
...
[`config_from_object()`](../reference/celery.html#celery.Celery.config_from_object "celery.Celery.config_from_object") is called. If you want to set additional
configuration you should do so after.

#### Example 1: Using the name of a module

The [`app.config_from_object()`](../reference/celery.html#celery.Celery.config_from_object "celery.Celery.config_from_object") method can take the fully qualified
...
The [`app.config_from_envvar()`](../reference/celery.html#celery.Celery.config_from_envvar "celery.Celery.config_from_envvar") takes the configuration module name
...
one is [`humanize()`](../reference/celery.app.utils.html#celery.app.utils.Settings.humanize "celery.app.utils.Settings.humanize"):
...
can use the [`table()`](../reference/celery.app.utils.html#celery.app.utils.Settings.table "celery.app.utils.Settings.table") method:
...
Creating a [`Celery`](../reference/celery.html#celery.Celery "celery.Celery") instance will only do the following:
...
> 4. Call the [`app.on_init()`](../reference/celery.html#celery.Celery.on_init "celery.Celery.on_init") callback (does nothing by default).

The [`app.task()`](../reference/celery.html#celery.Celery.task "celery.Celery.task") decorators don’t create the tasks at the point when
...
[`app.finalize()`](../reference/celery.html#celery.Celery.finalize "celery.Celery.finalize") – or implicitly by accessing the `app.tasks`
attribute.
...
Internally Celery uses the [`celery.app.app_or_default()`](../reference/celery.app.html#celery.app.app_or_default "celery.app.app_or_default") function
...
All tasks created using the [`app.task()`](../reference/celery.html#celery.Celery.task "celery.Celery.task") decorator
will inherit from the application’s base [`Task`](../reference/celery.app.task.html#celery.app.task.Task "celery.app.task.Task") class.
...
class `celery.Task` is only present for reference. For optimization,
...
To realize a base class you need to create a task using the [`app.task()`](../reference/celery.html#celery.Celery.task "celery.Celery.task")
decorator:
...
by changing its [`app.Task()`](../reference/celery.app.task.html#celery.app.task.Task "celery.app.task.Task") attribute:
