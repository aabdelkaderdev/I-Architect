<!-- Source: https://docs.celeryq.dev/en/main/userguide/security.html -->

This document describes the current stable version of Celery (5.6).
For development docs,
[go here](https://docs.celeryq.dev/en/main/userguide/security.html).
...
been tampered with. See [Message Signing](#id2) for information on how to make
...
and authentication using [`broker_use_ssl`](configuration.html#std-setting-broker_use_ssl).
...
any memory copied as a result of the `fork()` call,
...
but for the same reasons pickle is inherently insecure [[\*]](#id4),
and should be avoided whenever clients are untrusted or
unauthenticated.

You can disable untrusted content by specifying
a white-list of accepted content-types in the [`accept_content`](configuration.html#std-setting-accept_content)
setting:
...
authenticity of senders, to enable this read [Message Signing](#message-signing)
for more information.

## 

Celery can use the <https://pypi.org/project/cryptography/> library to sign message using
...
To enable this you should configure the [`task_serializer`](configuration.html#std-setting-task_serializer)
...
the [`security_key`](configuration.html#std-setting-security_key),
[`security_certificate`](configuration.html#std-setting-security_certificate), and [`security_cert_store`](configuration.html#std-setting-security_cert_store)
settings respectively.
You can tweak the signing algorithm with [`security_digest`](configuration.html#std-setting-security_digest).
If using an encrypted private key, the password can be configured with
[`security_key_password`](configuration.html#std-setting-security_key_password).

With these configured it’s also necessary to call the
`celery.setup_security()` function. Note that this will also
...
[rsyslog](http://www.rsyslog.com/)). Celery uses the [`logging`](https://docs.python.org/dev/library/logging.html#module-logging "(in Python v3.15)") library, and already has
