<!-- Source: https://docs.celeryq.dev/en/main/userguide/extending.html -->

This document describes the current stable version of Celery (5.6).
For development docs,
[go here](https://docs.celeryq.dev/en/main/userguide/extending.html).
...
For that purpose a special `ConsumerStep` bootstep class
exists, where you only need to define the `get_consumers` method, that must
return a list of [`kombu.Consumer`](https://docs.celeryq.dev/projects/kombu/en/main/reference/kombu.html#kombu.Consumer "(in Kombu v5.6)") objects to start
...
The [`WorkController`](../reference/celery.worker.html#celery.worker.WorkController "celery.worker.WorkController") is the core worker implementation,
...
blueprint
:   This is the worker [`Blueprint`](../reference/celery.bootsteps.html#celery.bootsteps.Blueprint "celery.bootsteps.Blueprint").

hub
:   Event loop object ([`Hub`](https://docs.celeryq.dev/projects/kombu/en/main/reference/kombu.asynchronous.html#kombu.asynchronous.Hub "(in Kombu v5.6)")). You can use
    this to register callbacks in the event loop.

    This is only supported by async I/O enabled transports (amqp, redis),
    in which case the worker.use\_eventloop attribute should be set.
...
:   The current process/eventlet/gevent/thread pool.
    See [`celery.concurrency.base.BasePool`](../internals/reference/celery.concurrency.base.html#celery.concurrency.base.BasePool "celery.concurrency.base.BasePool").
...
timer
:   [`Timer`](https://docs.celeryq.dev/projects/kombu/en/main/reference/kombu.asynchronous.timer.html#kombu.asynchronous.timer.Timer "(in Kombu v5.6)") used to schedule functions.
...
statedb
:   `` Database <celery.worker.state.Persistent>` `` to persist state between
    worker restarts.
...
autoscaler
:   `Autoscaler` used to automatically grow
...
controller
:   The parent [`WorkController`](../reference/celery.worker.html#celery.worker.WorkController "celery.worker.WorkController") object that created this consumer.
...
connection
:   The current broker connection ([`kombu.Connection`](https://docs.celeryq.dev/projects/kombu/en/main/reference/kombu.html#kombu.Connection "(in Kombu v5.6)")).
...
:   Worker to worker broadcast communication
    ([`Gossip`](../reference/celery.worker.consumer.gossip.html#celery.worker.consumer.gossip.Gossip "celery.worker.consumer.gossip.Gossip")).
    ...
    **Callbacks**
    ...
    - `<set> gossip.on.node_join`
      ...
      > [`Worker`](../reference/celery.events.state.html#celery.events.state.Worker "celery.events.state.Worker") instance.
    - `<set> gossip.on.node_leave`

      > Called whenever a new node leaves the cluster (shuts down),
      > providing a [`Worker`](../reference/celery.events.state.html#celery.events.state.Worker "celery.events.state.Worker") instance.
...
:   Responsible for sending worker event heartbeats
    ([`Heart`](../internals/reference/celery.worker.heartbeat.html#celery.worker.heartbeat.Heart "celery.worker.heartbeat.Heart")).
...
task\_consumer
:   The [`kombu.Consumer`](https://docs.celeryq.dev/projects/kombu/en/main/reference/kombu.html#kombu.Consumer "(in Kombu v5.6)") object used to consume task messages.
...
task\_buckets
:   A [`defaultdict`](https://docs.python.org/dev/library/collections.html#collections.defaultdict "(in Python v3.15)") used to look-up the rate limit for
    a task by type.
    Entries in this dict may be None (for no limit) or a
    [`TokenBucket`](https://docs.celeryq.dev/projects/kombu/en/main/reference/kombu.utils.limits.html#kombu.utils.limits.TokenBucket "(in Kombu v5.6)") instance implementing
...
qos
:   The `QoS` object can be used to change the
    task channels current prefetch\_count value:
...
consumer.reset\_rate\_limits()
:   Updates the `task_buckets` mapping for all registered task types.

consumer.bucket\_for\_task(*type*, *Bucket=TokenBucket*)
:   Creates rate limit bucket for a task using its `task.rate_limit`
    attribute.

consumer.add\_task\_queue(name, exchange=None, exchange\_type=None,

routing\_key=None, \\*\\*options):
:   Adds new queue to consume from. This will persist on connection restart.

consumer.cancel\_task\_queue(*name*)
:   Stop consuming from queue by name. This will persist on connection
    restart.

apply\_eta\_task(*request*)
:   Schedule ETA task to execute based on the `request.eta` attribute.
    ([`Request`](../reference/celery.worker.request.html#celery.worker.request.Request "celery.worker.request.Request"))
...
Logging with the Python logging module isn’t [reentrant](../glossary.html#term-reentrant):
meaning you cannot interrupt the function then
call it again later. It’s important that the `stop` and `shutdown` methods
you write is also [reentrant](../glossary.html#term-reentrant).

Starting the worker with [`--loglevel=debug`](../reference/cli.html#cmdoption-celery-worker-l)
...
`events` commands by modifying the [`user_options`](../reference/celery.html#celery.Celery.user_options "celery.Celery.user_options") attribute of the
application instance.

Celery commands uses the `click` module to parse command-line
arguments, and so to add custom arguments you need to add [`click.Option`](https://click.palletsprojects.com/en/stable/api/#click.Option "(in Click v8.3.x)") instances
to the relevant set.
...
The **celery** umbrella command supports the concept of ‘preload
...
New commands can be added to the **celery** umbrella command by using
[setuptools entry-points](http://reinout.vanrees.org/weblog/2010/01/06/zest-releaser-entry-points.html).

Entry-points is special meta-data that can be added to your packages `setup.py` program,
and then after installation, read from the system using the [`importlib`](https://docs.python.org/dev/library/importlib.html#module-importlib "(in Python v3.15)") module.
...
This is how the <https://pypi.org/project/Flower/> monitoring extension may add the **celery flower** command,
...
In the module `flower/command.py`, the command function may be defined
as the following:
...
### [`Hub`](https://docs.celeryq.dev/projects/kombu/en/main/reference/kombu.asynchronous.html#kombu.asynchronous.Hub "(in Kombu v5.6)") - The workers async event loop
...
hub.add\_writer(*fd*, *callback*, *\\*args*)
:   Add callback to be called when `fd` is writable.
    See also notes for [`hub.add_reader()`](#hub.add_reader "hub.add_reader") above.
...
timer.call\_after(secs, callback, args=(), kwargs=(),

priority=0)

timer.call\_repeatedly(secs, callback, args=(), kwargs=(),

priority=0)

timer.call\_at(eta, callback, args=(), kwargs=(),
