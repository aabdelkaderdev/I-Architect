<!-- Source: https://docs.celeryq.dev/en/main/userguide/optimizing.html -->
...
The point of this exercise [[\*]](#id4) is to show that there’s a limit
...
A way to do this is by [using Munin](monitoring.html#monitoring-munin).
...
You can tweak the [`broker_pool_limit`](configuration.html#std-setting-broker_pool_limit) setting to minimize
...
or by using [`task_routes`](configuration.html#std-setting-task_routes):
...
argument (or use the [`task_routes`](configuration.html#std-setting-task_routes) setting):

```
task.apply_async(args, queue='transient')
```

For more information see the [routing guide](routing.html#guide-routing).
...
available worker nodes that may be able to process them sooner [[†]](#id5),
or that the messages may not even fit in memory.

The workers’ default prefetch count is the
[`worker_prefetch_multiplier`](configuration.html#std-setting-worker_prefetch_multiplier) setting multiplied by the number
of concurrency slots [[‡]](#id6) (processes/threads/green-threads).
...
the tasks according to the run-time (see [Routing Tasks](routing.html#guide-routing)).

### Reserve one task at a time

The task message is only deleted from the queue after the task is
[acknowledged](../glossary.html#term-acknowledged), so if the worker crashes before acknowledging the task,
...
Notes at [Should I use retry or acks\_late?](../faq.html#faq-acks-late-vs-retry).
...
[`-c 10`](../reference/cli.html#cmdoption-celery-worker-c), the worker may reserve at most 20
...
late (10 unacknowledged tasks executing for [`-c 10`](../reference/cli.html#cmdoption-celery-worker-c))

For that, you need to enable [late acknowledgment](../glossary.html#term-late-acknowledgment). Using this option over the
...
abruptly, so this also means the task must be [idempotent](../glossary.html#term-idempotent)
...
prefetching by enabling [`worker_disable_prefetch`](configuration.html#std-setting-worker_disable_prefetch). With this
...
[`--disable-prefetch`](../reference/cli.html#cmdoption-celery-worker-disable-prefetch). This feature
...
[`worker_max_tasks_per_child`](configuration.html#std-setting-worker_max_tasks_per_child) and [`worker_max_memory_per_child`](configuration.html#std-setting-worker_max_memory_per_child)
settings.
...
tasks. For example, if you use a [`worker_max_tasks_per_child`](configuration.html#std-setting-worker_max_tasks_per_child) of 1
...
[`worker_max_memory_per_child`](configuration.html#std-setting-worker_max_memory_per_child).
