<!-- Source: https://docs.celeryq.dev/en/main/userguide/calling.html -->
...
used by task instances and the [canvas](canvas.html#guide-canvas).
...
The [`delay()`](../reference/celery.app.task.html#celery.app.task.Task.delay "celery.app.task.Task.delay") method is convenient as it looks like calling a regular
function:
...
Using [`apply_async()`](../reference/celery.app.task.html#celery.app.task.Task.apply_async "celery.app.task.Task.apply_async") instead you have to write:
...
See [chord error handling](canvas.html#chord-errors) for more information.
...
taken to ease the workload. See [Munin](monitoring.html#monitoring-munin).
...
exceeds visibility\_timeout (see [Caveats](../getting-started/backends-and-brokers/redis.html#redis-caveats)).
...
database-backed periodic tasks, e.g. with <https://pypi.org/project/django-celery-beat/> if
using Django (see [Using custom scheduler classes](periodic-tasks.html#beat-custom-schedulers)).
...
the task as [`REVOKED`](tasks.html#std-state-REVOKED) ([`TaskRevokedError`](../reference/celery.exceptions.html#celery.exceptions.TaskRevokedError "celery.exceptions.TaskRevokedError")).
...
the connection cannot be initiated, an [`OperationalError`](../reference/celery.exceptions.html#celery.exceptions.OperationalError "kombu.exceptions.OperationalError")
...
If you have [retries](#calling-retry) enabled this will only happen after
...
limits. Enable `confirm_publish` in [`broker_transport_options`](configuration.html#std-setting-broker_transport_options)
to detect this.
...
change this using the [`task_serializer`](configuration.html#std-setting-task_serializer) setting,
...
:   a standard part of Python (since 2.6), and is fairly fast to decode.
    ...
    With more complex workflows created using [Canvas: Designing Work-flows](canvas.html#guide-canvas), the JSON
...
[`accept_content`](configuration.html#std-setting-accept_content). By default, only JSON is accepted,
...
> 2. The [`Task.serializer`](../reference/celery.app.task.html#celery.app.task.Task.serializer "celery.app.task.Task.serializer") attribute
> 3. The [`task_serializer`](configuration.html#std-setting-task_serializer) setting.
...
> 3. The [`task_compression`](configuration.html#std-setting-task_compression) attribute.
...
the workers [`-Q`](../reference/cli.html#cmdoption-celery-worker-Q) argument:
...
is to use configuration routers ([`task_routes`](configuration.html#std-setting-task_routes)).

To find out more about routing, please see [Routing Tasks](routing.html#guide-routing).

## 

You can enable or disable result storage using the [`task_ignore_result`](configuration.html#std-setting-task_ignore_result)
...
set the [`result_extended`](configuration.html#std-setting-result_extended) setting to `True`.
...
For example, some integrations (e.g. <https://pypi.org/project/django-celery-beat/> together with
<https://pypi.org/project/django-celery-results/>) may record the *periodic task name* in the result
...
For more information on tasks, please see [Tasks](tasks.html#guide-tasks).
...
[routing guide](routing.html#guide-routing).

- exchange

  > Name of exchange (or a [`kombu.entity.Exchange`](../internals/reference/celery.backends.rpc.html#celery.backends.rpc.RPCBackend.Exchange "kombu.entity.Exchange")) to
  > send the message to.
- routing\_key
