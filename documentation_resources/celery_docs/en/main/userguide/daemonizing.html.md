<!-- Source: https://docs.celeryq.dev/en/main/userguide/daemonizing.html -->

This document describes the current stable version of Celery (5.6).
For development docs,
[go here](https://docs.celeryq.dev/en/main/userguide/daemonizing.html).
...
[our systemd documentation](#daemon-systemd-generic) for guidance.
...
you may want to refer to [our init.d documentation](#daemon-generic).
...
instead they can use the **celery multi** utility (or
...
also sets a default value for [`DJANGO_SETTINGS_MODULE`](http://docs.djangoproject.com/en/dev/topics/settings/#envvar-DJANGO_SETTINGS_MODULE "(in Django v6.1)")
as shown in the example Django project in [First steps with Django](../django/first-steps-with-django.html#django-first-steps).

#### 

- `CELERY_APP`

  > App instance to use (value for [`--app`](../reference/cli.html#cmdoption-celery-A) argument).
- `CELERY_BIN`
...
- `CELERYBEAT_OPTS`
  ...
  > Additional arguments to **celery beat**, see
  > **celery beat --help** for a list of available options.
- `CELERYBEAT_PID_FILE`
...
the `C_FAKEFORK` environment variable to skip the
daemonization step:
...
should [report it](../contributing.html#reporting-bugs)).
...
Once you’ve put that file in `/etc/systemd/system`, you should run
...
in the `[Unit]` [systemd section](https://www.freedesktop.org/software/systemd/man/systemd.unit.html#%5BUnit%5D%20Section%20Options).
...
When running as root without `C_FORCE_ROOT` the worker will
