<!-- Source: https://docs.celeryq.dev/projects/pytest-celery/en/latest/getting-started/first-steps.html -->

This document describes the current stable version of pytest\_celery (1.3).
For development docs,
[go here](https://pytest-celery.readthedocs.io/en/main/getting-started/first-steps.html).
...
If you are new to Celery, you should read the [Celery documentation](http://docs.celeryproject.org/en/latest/) first.
If you are new to pytest, you should read the [pytest documentation](https://docs.pytest.org/en/latest/) first.
...
By default, every test case is executed against all of the (enabled) [built-in vendors](vendors.html#vendors),
...
[`CeleryTestContainer`](../reference/pytest_celery.api.html#pytest_celery.api.container.CeleryTestContainer "pytest_celery.api.container.CeleryTestContainer").
...
- [`Worker Container`](../reference/pytest_celery.vendors.worker.html#pytest_celery.vendors.worker.container.CeleryWorkerContainer "pytest_celery.vendors.worker.container.CeleryWorkerContainer"): Base class for Celery worker containers.
- [`RabbitMQ Container`](../reference/pytest_celery.vendors.rabbitmq.html#pytest_celery.vendors.rabbitmq.container.RabbitMQContainer "pytest_celery.vendors.rabbitmq.container.RabbitMQContainer"): Base class for RabbitMQ containers.
- [`Redis Container`](../reference/pytest_celery.vendors.redis.html#pytest_celery.vendors.redis.container.RedisContainer "pytest_celery.vendors.redis.container.RedisContainer"): Base class for Redis containers.

These classes are Context-aware, and can be used to represent a container for their domain in the environment.

#### Test Nodes

A test node is the logical representation of a [test container](#test-containers).
...
[`CeleryTestNode`](../reference/pytest_celery.api.html#pytest_celery.api.base.CeleryTestNode "pytest_celery.api.base.CeleryTestNode").

There are three main types of nodes.

- [`Test Worker`](../reference/pytest_celery.api.html#pytest_celery.api.worker.CeleryTestWorker "pytest_celery.api.worker.CeleryTestWorker"): Represents a Celery worker instance.
- [`Test Broker`](../reference/pytest_celery.api.html#pytest_celery.api.broker.CeleryTestBroker "pytest_celery.api.broker.CeleryTestBroker"): Represents a broker instance.
- [`Test Backend`](../reference/pytest_celery.api.html#pytest_celery.api.backend.CeleryTestBackend "pytest_celery.api.backend.CeleryTestBackend"): Represents a result backend instance.
...
[`CeleryTestCluster`](../reference/pytest_celery.api.html#pytest_celery.api.base.CeleryTestCluster "pytest_celery.api.base.CeleryTestCluster").
...
- [`Worker Cluster`](../reference/pytest_celery.api.html#pytest_celery.api.worker.CeleryWorkerCluster "pytest_celery.api.worker.CeleryWorkerCluster"): Represents a cluster of Celery workers.
- [`Broker Cluster`](../reference/pytest_celery.api.html#pytest_celery.api.broker.CeleryBrokerCluster "pytest_celery.api.broker.CeleryBrokerCluster"): Represents a cluster of brokers.
- [`Backend Cluster`](../reference/pytest_celery.api.html#pytest_celery.api.backend.CeleryBackendCluster "pytest_celery.api.backend.CeleryBackendCluster"): Represents a cluster of result backends.
...
[`CeleryTestSetup`](../reference/pytest_celery.api.html#pytest_celery.api.setup.CeleryTestSetup "pytest_celery.api.setup.CeleryTestSetup").
...
The currently available vendors and their status are described in the [vendors](vendors.html#vendors) section.

Each vendor can be tested separately, for independent testing of each component.

For example, testing the [default redis broker](#redis-broker) container by itself.
...
Or, testing the [default redis broker](#redis-broker) at the node level.
...
|  |  |
| --- | --- |
| **Core Components** | The environment is built around three key components: the Celery worker, a broker, and a backend, each within their respective containers, nodes, and clusters. |
| **Lifecycle Management** | Containers and nodes manage the lifecycle of components, assembling them into clusters for the test setup. |
| **Independent Nodes** | Nodes function independently, enhancing flexibility in testing, configuration, and customization for specific project requirements. |
| **Main Entry Point** | All of the given components integrate into the test setup, which is the main entry point for the test environment. |
| **Modular Approach** | All nodes are interchangeable within their domain, allowing plug-and-play style configuration when setting up the test environment. |
| **Isoalted Environments** | Each test case has its own instances of the environment, allowing for parallelism and isolation of test cases. |
...
Each component of the setup has its own [parameterized fixtures set](https://docs.pytest.org/en/latest/how-to/parametrize.html).
...
These fixtures will generate a cluster with a single Celery worker node, based on the [built-in Dockerfile](vendors.html#built-in-worker).

1. [`celery_worker`](../reference/pytest_celery.fixtures.html#pytest_celery.fixtures.worker.celery_worker "pytest_celery.fixtures.worker.celery_worker"): Latest Celery worker node.
2. [`celery_worker_cluster`](../reference/pytest_celery.fixtures.html#pytest_celery.fixtures.worker.celery_worker_cluster "pytest_celery.fixtures.worker.celery_worker_cluster"): Single worker cluster for `celery_worker`.

#### Celery Application

The Celery app can be controlled by hooking into the [`default_worker_app`](../reference/pytest_celery.vendors.worker.html#pytest_celery.vendors.worker.fixtures.default_worker_app "pytest_celery.vendors.worker.fixtures.default_worker_app") fixture.
...
And every test in the [scope of this fixture](https://docs.pytest.org/en/latest/reference/fixtures.html#fixture-availability) will use the modified app.
...
The available tasks can be controlled by hooking into the [`default_worker_tasks`](../reference/pytest_celery.vendors.worker.html#pytest_celery.vendors.worker.fixtures.default_worker_tasks "pytest_celery.vendors.worker.fixtures.default_worker_tasks") fixture.
The plugin adds a [`ping task`](../reference/pytest_celery.vendors.worker.html#pytest_celery.vendors.worker.tasks.ping "pytest_celery.vendors.worker.tasks.ping") by default, but you can add your own tasks like this:
...
the [`default_worker_signals`](../reference/pytest_celery.vendors.worker.html#pytest_celery.vendors.worker.fixtures.default_worker_signals "pytest_celery.vendors.worker.fixtures.default_worker_signals") fixture.
...
These fixtures will generate a cluster with a single broker node, for each enabled broker [vendor](vendors.html#vendors).
The test case will be parameterized for each available broker.

1. [`celery_broker`](../reference/pytest_celery.fixtures.html#pytest_celery.fixtures.broker.celery_broker "pytest_celery.fixtures.broker.celery_broker"): Parameterized fixture for all of the available brokers nodes.
2. [`celery_broker_cluster`](../reference/pytest_celery.fixtures.html#pytest_celery.fixtures.broker.celery_broker_cluster "pytest_celery.fixtures.broker.celery_broker_cluster"): Single broker cluster for `celery_broker`.

The [RabbitMQ Management Example](../userguide/examples/rabbitmq_management.html#examples-rabbitmq-management) provides a good demonstration of how to override the default broker configuration,
with a single `rabbitmq:management` broker instead of the broker matrix.

### 

These fixtures will generate a cluster with a single backend node, for each enabled backend [vendor](vendors.html#vendors).
The test case will be parameterized for each available backend.

1. [`celery_backend`](../reference/pytest_celery.fixtures.html#pytest_celery.fixtures.backend.celery_backend "pytest_celery.fixtures.backend.celery_backend"): Parameterized fixture for all of the available backends nodes.
2. [`celery_backend_cluster`](../reference/pytest_celery.fixtures.html#pytest_celery.fixtures.backend.celery_backend_cluster "pytest_celery.fixtures.backend.celery_backend_cluster"): Single backend cluster for `celery_backend`.
...
Eventually, all of the components will be combined into the [`celery_setup`](../reference/pytest_celery.fixtures.html#pytest_celery.fixtures.setup.celery_setup "pytest_celery.fixtures.setup.celery_setup") fixture, as
discussed [before](#test-setup).
...
This section, introduced us to the plugin’s mechanism to manipulate and configure those components using their [default fixtures](#default-fixtures).

Let’s have a quick recap over what we just learned in this section then.

|  |  |
| --- | --- |
| **Parameterized Fixtures** | Each component of the setup has its own parameterized fixtures set, one for the node and one for the cluster. |
| **Configurable Components** | Each default component has its `default_` fixtures list, which can be used to control or extend the component’s functionality. |
| **Setup Matrix** | The [`celery_setup`](../reference/pytest_celery.fixtures.html#pytest_celery.fixtures.setup.celery_setup "pytest_celery.fixtures.setup.celery_setup") will generate a [Test Setup Matrix](../userguide/setup-matrix.html#setup-matrix) of isolated environments for each test case, based on the enabled components and their configurations. |
...
Use the [Examples](../userguide/examples/index.html#examples) as a starting point for your POCs; it can be very useful for practicing the concepts of this guide.
...
For additional documentation, see [pytest-docker-tools documentation](https://github.com/Jc2k/pytest-docker-tools?tab=readme-ov-file#containers).
...
All of the built-in nodes are based on the [Test Nodes](#test-nodes) classes.
...
It uses the latest Celery release and its own [Dockerfile](vendors.html#built-in-worker).

##### Container

The [`CeleryWorkerContainer`](../reference/pytest_celery.vendors.worker.html#pytest_celery.vendors.worker.container.CeleryWorkerContainer "pytest_celery.vendors.worker.container.CeleryWorkerContainer") is used
to describe the [Built-in Celery Worker](vendors.html#built-in-worker).

##### Node

The [`CeleryTestWorker`](../reference/pytest_celery.api.html#pytest_celery.api.worker.CeleryTestWorker "pytest_celery.api.worker.CeleryTestWorker") is used to represent the worker node.

##### Fixtures

A list of available fixtures for the worker can be found in the [`pytest_celery.vendors.worker.fixtures`](../reference/pytest_celery.vendors.worker.html#module-pytest_celery.vendors.worker.fixtures "pytest_celery.vendors.worker.fixtures") module.
...
The [`RabbitMQContainer`](../reference/pytest_celery.vendors.rabbitmq.html#pytest_celery.vendors.rabbitmq.container.RabbitMQContainer "pytest_celery.vendors.rabbitmq.container.RabbitMQContainer") is used
to describe the `rabbitmq:latest` docker image.

##### Node

The [`RabbitMQTestBroker`](../reference/pytest_celery.vendors.rabbitmq.html#pytest_celery.vendors.rabbitmq.api.RabbitMQTestBroker "pytest_celery.vendors.rabbitmq.api.RabbitMQTestBroker") is used to represent the broker node.

##### Fixtures

A list of available fixtures for the broker can be found in the [`pytest_celery.vendors.rabbitmq.fixtures`](../reference/pytest_celery.vendors.rabbitmq.html#module-pytest_celery.vendors.rabbitmq.fixtures "pytest_celery.vendors.rabbitmq.fixtures") module.
...
The [`RedisContainer`](../reference/pytest_celery.vendors.redis.html#pytest_celery.vendors.redis.container.RedisContainer "pytest_celery.vendors.redis.container.RedisContainer") is used
to describe the `redis:latest` docker image.

##### Node

The [`RedisTestBroker`](../reference/pytest_celery.vendors.redis.broker.html#pytest_celery.vendors.redis.broker.api.RedisTestBroker "pytest_celery.vendors.redis.broker.api.RedisTestBroker") is used to represent the broker node.

##### Fixtures

A list of available fixtures for the broker can be found in the [`pytest_celery.vendors.redis.broker.fixtures`](../reference/pytest_celery.vendors.redis.broker.html#module-pytest_celery.vendors.redis.broker.fixtures "pytest_celery.vendors.redis.broker.fixtures") module.
...
The [`LocalstackContainer`](../reference/pytest_celery.vendors.localstack.html#pytest_celery.vendors.localstack.container.LocalstackContainer "pytest_celery.vendors.localstack.container.LocalstackContainer") is used
to describe the `localstack/localstack` docker image.

##### Node

The [`LocalstackTestBroker`](../reference/pytest_celery.vendors.localstack.html#pytest_celery.vendors.localstack.api.LocalstackTestBroker "pytest_celery.vendors.localstack.api.LocalstackTestBroker") is used to represent the broker node.

##### Fixtures

A list of available fixtures for the broker can be found in the [`pytest_celery.vendors.localstack.fixtures`](../reference/pytest_celery.vendors.localstack.html#module-pytest_celery.vendors.localstack.fixtures "pytest_celery.vendors.localstack.fixtures") module.
...
The [`RedisTestBackend`](../reference/pytest_celery.vendors.redis.backend.html#pytest_celery.vendors.redis.backend.api.RedisTestBackend "pytest_celery.vendors.redis.backend.api.RedisTestBackend") is used to represent the backend node.

##### Fixtures

A list of available fixtures for the backend can be found in the [`pytest_celery.vendors.redis.backend.fixtures`](../reference/pytest_celery.vendors.redis.backend.html#module-pytest_celery.vendors.redis.backend.fixtures "pytest_celery.vendors.redis.backend.fixtures") module.
...
The [`MemcachedContainer`](../reference/pytest_celery.vendors.memcached.html#pytest_celery.vendors.memcached.container.MemcachedContainer "pytest_celery.vendors.memcached.container.MemcachedContainer") is used
to describe the `memcached:latest` docker image.

##### Node

The [`MemcachedTestBackend`](../reference/pytest_celery.vendors.memcached.html#pytest_celery.vendors.memcached.api.MemcachedTestBackend "pytest_celery.vendors.memcached.api.MemcachedTestBackend") is used to represent the backend node.

##### Fixtures

A list of available fixtures for the backend can be found in the [`pytest_celery.vendors.memcached.fixtures`](../reference/pytest_celery.vendors.memcached.html#module-pytest_celery.vendors.memcached.fixtures "pytest_celery.vendors.memcached.fixtures") module.

Warning

The Memcached backend component is current experimental.

Please [report](help.html#help) any issues you encounter!

### 

In the previous sections, we’ve covered which components compose the Celery test environment and how to construct
your own setup and configurations. We’ve seen [Examples](../userguide/examples/index.html#examples) that are using the built-in components and in this section,
...
|  |  |
| --- | --- |
| **Built-in Vendors** | The plugin provides a worker, borker and backend components out-of-the-box and generates a matrix of all possible combinations for each test case by default. |
| **Component APIs** | Each component has container and node classes that provides an API for interacting with the component in the test case. |
| **Component Fixtures** | Each component has a `default_` fixtures list that can be used to control or extend the component’s functionality. |
| **Extensible Design** | Most of the configurations can be overridden or extended to bootstrap the environment for the target project. |
...
[Using the @shared\_task decorator](https://docs.celeryq.dev/en/stable/django/first-steps-with-django.html#using-the-shared-task-decorator).
...
Try [Disabling the Backend](#disable-backend) to see how it affects the test case. Don’t forget to remove the `get()` call.
...
in the [standard examples](../userguide/examples/index.html#examples) section. Try to get it running on your own and experiment with it.
...
If you want to learn more you should continue to the [Next Steps](next-steps.html#next-steps) tutorial, and after that you
can read the [User Guide](../userguide/index.html#userguide).