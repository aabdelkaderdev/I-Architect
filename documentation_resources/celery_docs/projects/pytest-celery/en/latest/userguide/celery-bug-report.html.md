<!-- Source: https://docs.celeryq.dev/projects/pytest-celery/en/latest/userguide/celery-bug-report.html -->

This document describes the current stable version of pytest\_celery (1.3).
For development docs,
[go here](https://pytest-celery.readthedocs.io/en/main/userguide/celery-bug-report.html).
...
[bug report issue](https://github.com/celery/celery/issues/new/choose).
...
When reporting a bug, you want to have the most simple and specific reproduction environment. To disable the [Test Setup Matrix](setup-matrix.html#setup-matrix),
...
[`default_rabbitmq_broker_image`](../reference/pytest_celery.vendors.rabbitmq.html#pytest_celery.vendors.rabbitmq.fixtures.default_rabbitmq_broker_image "pytest_celery.vendors.rabbitmq.fixtures.default_rabbitmq_broker_image") like this:
...
To use the `rabbitmq:management` label, see the [rabbitmq\_management](examples/rabbitmq_management.html#examples-rabbitmq-management) example.
...
[`default_redis_broker_image`](../reference/pytest_celery.vendors.redis.broker.html#pytest_celery.vendors.redis.broker.fixtures.default_redis_broker_image "pytest_celery.vendors.redis.broker.fixtures.default_redis_broker_image") like this:
...
Decide **if a** backend is needed and [disable the default backend](../getting-started/first-steps.html#disable-backend) if it’s not needed or set an exact backend to match
...
[`default_redis_backend_image`](../reference/pytest_celery.vendors.redis.backend.html#pytest_celery.vendors.redis.backend.fixtures.default_redis_backend_image "pytest_celery.vendors.redis.backend.fixtures.default_redis_backend_image") like this:
...
[`default_memcached_backend_image`](../reference/pytest_celery.vendors.memcached.html#pytest_celery.vendors.memcached.fixtures.default_memcached_backend_image "pytest_celery.vendors.memcached.fixtures.default_memcached_backend_image") like this:
...
Use the [Built-in Celery Worker](../getting-started/vendors.html#built-in-worker) to use a custom version or use the smoke tests’s worker to use the source code version.
...
The [`default_worker_celery_version`](../reference/pytest_celery.vendors.worker.html#pytest_celery.vendors.worker.fixtures.default_worker_celery_version "pytest_celery.vendors.worker.fixtures.default_worker_celery_version") is used
...
To install the worker from source, just run the test script from the [t/smoke/tests](https://github.com/celery/celery/tree/main/t/smoke/tests) directory.

It will automatically set up a [dev](https://github.com/celery/celery/blob/main/t/smoke/workers/dev.py) worker for the test.

### 

The plugin provides [Built-in Tasks](default-tasks.html#default-tasks) by default. For example, to use the [`ping task`](../reference/pytest_celery.vendors.worker.html#pytest_celery.vendors.worker.tasks.ping "pytest_celery.vendors.worker.tasks.ping"),
...
To add new tasks, create a new `tasks.py` module and use the [`default_worker_tasks`](../reference/pytest_celery.vendors.worker.html#pytest_celery.vendors.worker.fixtures.default_worker_tasks "pytest_celery.vendors.worker.fixtures.default_worker_tasks") fixture
to inject the tasks into the worker as described in the [How to add tasks](tasks.html#injecting-tasks) section.
...
and smoke tests tasks, in addition to the [Built-in Tasks](default-tasks.html#default-tasks), so you can use them to reproduce a scenario as well.
...
to the [Signal handlers modules injection](signals.html#signal-handlers-modules-injection) section.
...
The snippet is also part of the [CI system](https://github.com/celery/pytest-celery/actions/workflows/examples.yml).
...
We’ll use the [Built-in Celery Worker](../getting-started/vendors.html#built-in-worker) to use a specific Celery release.

#### 

examples.celery\_bug\_report.py
...
The following snippet can be used as a starting point for a bug report script. To use it, just copy and paste it into a
new file in [t/smoke/tests](https://github.com/celery/celery/tree/main/t/smoke/tests) and run it with tox or pytest.
...
1. Create a new file in [t/smoke/tests](https://github.com/celery/celery/tree/main/t/smoke/tests), for example `test_issue_1234.py`.
2. Copy and paste the snippet into the new file.
