...
You are executing the `bmad-review-edge-case-hunter` workflow.
...
## Input Diff
Here is the diff output of the changes to review:
...
1. Walk every branching path and boundary condition within scope (control flow like conditionals, loops, error handlers, early returns; and domain boundaries where values, states, or conditions transition).
2. Collect only the unhandled paths as findings — discard handled ones silently.
