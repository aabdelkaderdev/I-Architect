...
_PROMPT_TEMPLATE = "saam_analysis.md"
...
def build_raa_a_subgraph() -> StateGraph:
    ...
    def extract_saam(state: StrategySubgraphState, config: RunnableConfig) -> dict:
        configurable = config.get("configurable", {})
        llm = configurable.get("raa_a_llm")
        batch = state["batch"]
...
def _extract_fragment(llm, prompt: str) -> ArchFragment:
    ...
    if llm is None:
        logger.warning("No raa_a_llm configured — returning empty ArchFragment")
        return ArchFragment(metadata={"strategy": "raa_a", "note": "no LLM configured"})

    try:
        structured = llm.with_structured_output(ArchFragment, include_raw=True)
        response = structured.invoke(prompt)
    except Exception:
        logger.exception("Structured extraction failed for RAA-A")
        return ArchFragment(metadata={"strategy": "raa_a", "error": "extraction_failed"})
    ...
    if isinstance(response, dict):
        if response.get("parsing_error"):
            logger.warning("RAA-A parsing error: %s", response["parsing_error"])
        parsed = response.get("parsed")
        if isinstance(parsed, ArchFragment):
            return parsed

    logger.warning("RAA-A extraction returned unexpected type: %s", type(response))
    return ArchFragment(metadata={"strategy": "raa_a", "error": "malformed_response"})
